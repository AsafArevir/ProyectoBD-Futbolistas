...
#main-content {
	...
	<meta charset="utf-8"/>
	<title>CRUD Auxiliares tecnicos</title>
</head>
<body>
	<button size=40px type="button" onclick="javascript:window.print()">Imprimir</button>
	<H1 align="center"><font color=white>CRUD Auxiliares tecnicos</font></H1>
 <form action="auxiliaresc.php" method="post">
 	...
 	<label>ID_auxiliar: </label><input type="text" name="idauxiliar">
 	<label>ID_equipo: </label><input type="text" name="idequipo">
	...
	if (isset($_POST['EJECUTAR'])) {
		...
		switch ($opcione) {
			case 'INSERTAR':
 					...
 					$sql = "INSERT INTO auxiliares values ('$id_auxiliar','$id_equipo', '$nombre', '$apellido')";
			...
			case 'ELIMINAR':
					$conn= oci_connect($user,$pass,$host);
 					$sql = "DELETE FROM auxiliares WHERE id_auxiliar='$id_auxiliar'";
 					$unir = oci_parse($conn,$sql);
  					oci_execute($unir);
			...
			case 'ACTUALIZAR':
					$conn= oci_connect($user,$pass,$host);
 					$sql = "UPDATE auxiliares SET id_equipo = '$id_equipo' WHERE id_auxiliar = '$id_auxiliar'";
 					$unir = oci_parse($conn,$sql);
  					oci_execute($unir);
